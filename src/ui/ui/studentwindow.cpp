...
StudentWindow::StudentWindow(GUIAuthManager &authManager,
...
void StudentWindow::on_enter_clicked()
{
    ...
    else if (this->ui->r3->isChecked())
    {
        ...
        try
        {
            std::vector<Thing> things = this->thingManager.viewStudentThings(this->studentNumber);
            ui->tableWidget->setColumnCount(4); // Указываем число колонок
            ui->tableWidget->setRowCount(0);
            ui->tableWidget->setColumnWidth(0, 50);
